...
<section id="detail_contents">
	<form name="myForm">
       ...
       <div class="btn_baordZone2">
       	   <a href="#" id="tempSaveBtn" onclick="fnObj.doSave('N');return false;" class="btn_blueblack btn_auth">임시저장</a>
...
var fnObj = {
    ...
    changeProjectDate : function(type){
    	...
    	g_beforeStartDate = $("#inStartDate").val();
    ...
    setTotalPrice : function(){
  	...
  	clickRdPeriod: function(obj){
  		...
  		if(obj.value == 'N'){
  			...
  			if(g_beforeStartDate == '' ) $("#inStartDate").datepicker('setDate', new Date());
			else $("#inStartDate").datepicker('setDate', g_beforeStartDate);
  			...
  			//if(g_projectConfirm != 'Y'){
  				...
  				if(g_beforecloseDate>'9990-01-03'){				//무기한으로 되어있다면 시작날짜 1달 후로 세팅
  					var nDate = new Date();
  	  				...
  	  				$("#closeDate").datepicker('setDate', new Date(Date.parse(nDate)+7 *1000*60*60*24));
  	  				if(g_beforeStartDate == '' ) $("#inStartDate").datepicker('setDate', new Date());
					else $("#inStartDate").datepicker('setDate', g_beforeStartDate);
  				}else{
    ...
    setProjectInfo: function(pId){
    	...
    	var callback = function(result){
    		...
    		if(obj.result == "SUCCESS"){
    	    	...
    	    	g_beforeStartDate = project.startDate;
