...
//직위 공통코드
var comCodeRankType = getBaseCommonCode('RANK', null, 'CD', 'NM', '', '직위선택','ALL', { orgId : "${userDetail.orgId}" });
var comCodeFamilyType = getBaseCommonCode('FAMILY_RELATION', null, 'CD', 'NM', '', '관계선택','ALL', { orgId : "${userDetail.orgId}" });
var comCodeSchoolTypeType = getBaseCommonCode('SCHOOL_TYPE', null, 'CD', 'NM', '', '선택','ALL', { orgId : "${userDetail.orgId}" });
var comCodeGraduateType = getBaseCommonCode('GRADUATE_TYPE', null, 'CD', 'NM', '', '선택','ALL', { orgId : "${userDetail.orgId}" });
var comCodeUserStatusType = getBaseCommonCode('USER_STTS', null, 'CD', 'NM', '', '선택','ALL', { orgId : "${userDetail.orgId}" });
var colorObj = {};
var now = new Date();
...
$(document).ready(function(){
	...
	//-------------------------- 모달창 :S -------------------------
   	...
   	//사용자 권한
	var params = {'orgId':"${userDetail.orgId }" , enable : 'Y'};
	roleIdCombo = getCodeInfo("${baseUserLoginInfo.lang}", 'CD', 'NM', '', '선택', 'SELECT', '/system/getRoleCodeCombo.do', params);		//권한코드(콤보용) 호출
	if(roleIdCombo == null){
		roleIdCombo = [{'CD': '', 'NM':'선택'}];
	}
	var htmlStr = createSelectTag('userRole', roleIdCombo, 'CD', 'NM', "${userDetail.roleId }", '', {}, '', 'select_b mgl6 w120px' );
...
function addCareer(){
	...
	$("div[name='careerRankArea']").eq(careerRankLength).html(comTag);
...
function addAcademic(){
	...
	$("span[name='schoolTypeArea']").eq(schoolLength).html(schoolTypeTag);
	...
	$("div[name='graduateTypeArea']").eq(graduateLength).html(graduateTypeTag);
...
function addFamily(){
	...
	$("div[name='relationNew']").eq(relationLength).html(familyTypeTag);
...
function makeReligionArea(){
	//혈액형 공통코드 생성
	var comCodeReligionType = getBaseCommonCode('RELIGION', null, 'CD', 'NM', null, '','ALL', { orgId : "${userDetail.orgId}" });
	//id/name , codeStr, value , text ,selectValue ,onchange....
...
function makeBloodArea(){
	//혈액형 공통코드 생성
	var comCode = getBaseCommonCode('BLOOD_RH', null, 'CD', 'NM', '', '선택','ALL', { orgId : "${userDetail.orgId}" });
	//id/name , codeStr, value , text ,selectValue ,onchange....
	...
	//혈액형 공통코드 생성
	var comCodeBloodType = getBaseCommonCode('BLOOD', null, 'CD', 'NM', null, '','ALL', { orgId : "${userDetail.orgId}" });
	//id/name , codeStr, value , text ,selectValue ,onchange....
...
function makeSosTelWhoSelect(){
	...
	var comCode = getBaseCommonCode('SOS_WHO', null, 'CD', 'NM', '', '선택','ALL', { orgId : "${userDetail.orgId}" });
	//id/name , codeStr, value , text ,selectValue ,onchange....
...
function makeUserStatusSelect(){
	//채용형태selectBox 생성
	var comCode = getBaseCommonCode('EMP_TYPE', null, 'CD', 'NM', '', '선택','ALL', { orgId : "${userDetail.orgId}" });
	//id/name , codeStr, value , text ,selectValue ,onchange....
	var comTag = createSelectTag('empType', comCode, 'CD', 'NM', '${userDetail.empType}', null, colorObj, 100, 'select_b w100pro');   //'fnObj.clickRdBudget(this);');//radio tag creator 함수 호출 (common.js)
        ...
        try {
            ...
            if (!document.getElementById("ie_preview_error_" + previewId)) {
               /*  var info = document.createElement("<p>");
                ...
                $("#previewId").html("미리보기를 지원하지 않는 브라우저입니다.")

               // alert("미리보기를 지원하지 않는 브라우저입니다.");
            }
        }
    } else { //ie가 아닐때
...
var fnObj = {
	    ...
	    concatDeptInchargeList:function(list){
	    	...
	    	//$("#deptArea").empty();
	    	$("#mainDept").empty();							//대표부서 선택 박스
			$("#mainDept").append("<option value = ''>선택</option>");
	   		//추가
	   		//var allCnt = $(".employee_list2").length;		//보이는것 임시삭제한것 포함 모든 것 갯수
	    	for(var i=0; i<list.length; i++){
    			...
    			var stStr = "<span class=\"employee_list2\">";
	    ...
	    openCompany : function(orgId){
	        //소속회사 팝업
	        var url = "../user/orgAuthCompanyListPop.do";
	        var params = {
	                targetOrgId : orgId     //$("#orgComId").val()
	        };
	        myComModal.open({
	            url: url,
	            pars: params,
...
function doSave(){
	...
	if($("#bloodRh").val()!="" || $("input[name='blood']:checked").length>0){
		if($("#bloodRh").val()==""){
			alert("Rh식 혈액형을 선택해 주세요.");
			$("#bloodRh").focus();
			return;
		}

		if($("input[name='blood']:checked").length==0){
			alert("혈액형을 선택해 주세요.");
			$("input[name='blood']").eq(0).focus();
			return;
		}
	}
	...
	if(confirm("저장하시겠습니까?")){
		...
		//imageFileUpload('', contextRoot+"/personnel/management/basicInfoImg.do", param);
...
function imageFileUpload(fileObj, url, paramObj){
	...
	commonAjaxForFileCreateForm(url,'','upFile','','', callback, true,paramObj);
...
//프로필 사진 삭제
function deleteProfileFile(){
	var url = contextRoot+"/personnel/management/deleteBasicInfoImg.do";
	var param = {
...
function tryInitPwd(){
	dialog.push({
	    body:'<b>Caution</b>&nbsp;&nbsp;비밀번호를 초기화 하시겠습니까?', top:200, type:'Caution', buttons:[
            {buttonValue:'확인', buttonClass:'Red', onClick:doInitPwd},	//, data:{param:"222"}},	//Red W100
