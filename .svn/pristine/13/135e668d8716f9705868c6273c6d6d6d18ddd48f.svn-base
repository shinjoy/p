...
<script type="text/javascript">
	...
	function linkPage(pageNo){
		...
		$("#frm").attr("action",contextRoot+"/approve/approveRefDocListAjax.do");
	...
	function goDetailPage(appvDocId,appvDocClass, appvDocType, docStatus){
		...
		$("#frm").attr("action",contextRoot+"/approve/getApproveRefDetail.do").submit();
	}
	//체크박스 전체 선택/해재
	function contentIdChkAll2($obj){
	...
	//체크박스 읽음 처리
	function readChkAll(){
		var url = contextRoot + "/approve/processReadAppvList.do";
