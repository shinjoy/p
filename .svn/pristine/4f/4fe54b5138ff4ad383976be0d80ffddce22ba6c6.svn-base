...
<div class="doc_viewWarp">
	...
	<div class="doc_viewWarp2">
		...
		<h3 class="print_h3title">교육신청서</h3>
		<table class="doc_print_tb_st01" summary="교육신청서 보기(작성자, 신청일, 소속(직위), 연락처, 휴가기간, 휴가기간, 휴가내용, 업무대행자, 대결자, 참조인)">
			<caption>교육신청서 보기</caption>
			<colgroup>
				<col width="120" />
				<col width="*" />
				<col width="120" />
			...
			<tbody>
				<tr>
					<th scope="row">대상자</th>
					<td>
					<span class='userProfileTargetArea' onmousedown = 'openUserProfileBox("${detailMap.userId}",$(this),"mouseover",null,0,0,0,0)'>
					...
					<tr>
						...
						<td colspan="3">
							<c:forEach items="${detailMap.approveLinkDocList }" var="data">
								<div onclick="openAppvDocPop('${data.appvDocId}','${data.appvDocClass}','${data.appvDocType}')" style="cursor: pointer;">
				...
				<tr>
					...
					<td>${detailMap.eduName}</td>
					<th scope="row">교육기관</th>
					<td>${detailMap.eduAgency}</td>
				</tr>
				<tr>
					<th scope="row">출근인정여부</th>
					<td>${detailMap.attendYn=='Y'?'출근인정':'출근미인정'}</td>
					<th scope="row">완료보고작성</th>
					<td>${detailMap.reportYn=='Y'?'작성':'미작성'}</td>
				</tr>
				<tr>
					<th scope="row">교육기간</th>
					<td colspan="3"><span class="txt_num"><fmt:formatDate value='${detailMap.dateFrom}' pattern='yyyy-MM-dd'/></span> (<fmt:formatDate value="${detailMap.dateFrom}" type="both" pattern="E"/>)
					~<span class="txt_num"><fmt:formatDate value='${detailMap.dateTo}' pattern='yyyy-MM-dd'/></span>
					(<fmt:formatDate value="${detailMap.dateTo}" type="both" pattern="E"/>)
					<span class="txt_num mgl15">
						<c:if test="${detailMap.hourFrom<10 }">0</c:if>${detailMap.hourFrom } :
						<c:if test="${detailMap.minuteFrom<10 }">0</c:if>${detailMap.minuteFrom } ~
						<c:if test="${detailMap.hourTo<10 }">0</c:if>${detailMap.hourTo } :
						<c:if test="${detailMap.minuteTo<10 }">0</c:if>${detailMap.minuteTo }
					</span>
					<span class="txt_num mgl15">(${detailMap.eduDay }</span>일간, 총<span class="txt_num">${detailMap.eduTime }</span>시간)</td>
				</tr>
				<tr>
					<th scope="row">금액</th>
					<td colspan="3"><fmt:formatNumber type="CURRENCY" pattern="#,##0" value="${detailMap.amount}" />원</td>
				</tr>
				<tr>
					<th scope="row">교육참여자</th>
					<td colspan="3">
					<c:forEach items="${detailMap.eduWorkerList }" var = "data" varStatus="i">
						<c:if test="${i.index !=0 }">
               				,
				...
				<tr>
					<th scope="row">참고사이트</th>
					<td colspan="3">
						<c:out value="${fn:replace(detailMap.siteUrl, LF, '<br>')}" escapeXml="false"/>
				...
				<tr>
					...
					<td>
						<span class='userProfileTargetArea' onmousedown = 'openUserProfileBox("${detailMap.workAgencyId}",$(this),"mouseover",null,0,0,0,0)'>
							<strong>${detailMap.workAgencyNm}</strong>
						</span>
					</td>
					<th scope="row">대결자</th>
					<td>
						<span class='userProfileTargetArea' onmousedown = 'openUserProfileBox("${detailMap.appvAgencyId}",$(this),"mouseover",null,0,0,0,0)'>
		...
		<p class="doc_des_st">위와 같이 교육 신청서를 제출합니다.</p>
