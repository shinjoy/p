...
<form name="myForm" method="post">
	<div class="mo_container">
		...
		<div class="sys_p_noti mgt10">
			...
			<p class="pointred mgr10 existChild"> 하위부서가 존재시 사용여부 수정불가, 해당부서 삭제 불가합니다.</p>
			<p class="pointred mgr10 existChild"> 부서이동은 트리 메뉴에서 변경 시켜주세요.</p>
		</div>
		<div class="btnZoneBox">
			...
			<c:if test="${mode ne 'new'}">
				<customTagUi:authBtn txt="삭제" orgId="${targetOrgId}" event="onclick='fnObj.tryDelete();'" ele="button" classValue="p_blueblack_btn" id="btnDel"/>
			</c:if>
			<button id="btnSave" type="button" class="p_withelin_btn btn_auth" onclick="fnObj.myWinClose();"><strong>닫기</strong></button>
		</div>
	</div>
</form>
...
var fnObj = {
	...
	preloadCode: function(){
		...
		if(mode=="new"){
			...
			var strHtml = "<input type='hidden' name='parentDeptId' id='parentDeptId' value='"+ this.parentDeptId +"'/>";
			$("#parentDeptSelectTag").html(strHtml);
		}
 ...
 */
    ...
    getDeptInfo: function(mode){
    	...
    	if(mode=="update" && deptInfoObj.length>0){
			...
			$(".existChild").show();     //하위부서에 관련한 문구
            $("#parentDeptId").hide();
            $("#parentDeptArea").append(deptInfo.parentDeptNm);
		}else{
