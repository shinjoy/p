...
<script type="text/javascript" defer="defer">
	...
	var commonPropject = getCommonProject('CD', 'NM', '', '${baseUserLoginInfo.projectTitle } 선택', { orgId : orgId,userId : "${baseUserLoginInfo.userId}",type:"PROJECT",incApproveActivity:"N",startDate:nowStr});
	...
	///////////////////////////////날짜 DATEPICKER : S///////////////////////////////
		...
		$('#dateFrom').datepicker("option", "onSelect", function ( selectedDate ) {
			commonPropject = getCommonProject('CD', 'NM', '', '${baseUserLoginInfo.projectTitle } 선택', { orgId : orgId,userId : "${baseUserLoginInfo.userId}",type:"PROJECT",incApproveActivity:"N",startDate:$("#dateFrom").val() });
		...
		var projectTag = createSelectTag('tripType', comCodeTripType, 'CD', 'NM', '${data.userStatus}', null, colorObj, null, 'select_b w100pro');   //'fnObj.clickRdBudget(this);');//radio tag creator 함수 호출 (common.js)
		$(".tripTypeArea").html(projectTag);
	...
	function doSave(){
		...
		var projectEndStr = $("#projectId option:selected").attr("lastDate");
		var projectEndBuf = projectEndStr.split("-").join("");
		var projectEndInt = parseInt(projectEndBuf);

		if(dateToInt > projectEndInt){
			alert("출장기간이 ${baseUserLoginInfo.projectTitle } 기간과 맞지 않습니다.\n기간을 다시 등록해주세요.\n[${baseUserLoginInfo.projectTitle } 기간: "+$("#projectId option:selected").attr("startDate")+" ~ "+$("#projectId option:selected").attr("lastDate")+"]");
			return;
		}
		...
		var activityEndStr = $("#activityId option:selected").attr("enddate");
		var activityEndBuf = activityEndStr.split("-").join("");
		var activityEndInt = parseInt(activityEndBuf);

		if(dateToInt > activityEndInt){
			confirmStr = "등록한 출장일정이 ${baseUserLoginInfo.activityTitle }의 일정을 초과합니다. 저장하시겠습니까?";
		}
