...
function CalAddCss(){
		...
		cssHtml += ".calImg{cursor:pointer;}</style>";
		$("head").append(cssHtml);
}
...
function initCal(obj){
	...
	var minYear 	= obj.min == null ? year_0-5 : obj.min;
	var maxYear 	= obj.max == null ? year_0+5 : obj.max;
	var splitKey 	= obj.splitKey == null ? "-" : obj.splitKey; 
	var todayYN		= obj.today == null ? "n" : "y";
	var iconYN		= obj.icon  == null ? "n" : "y";
	var iconUrl    = obj.iconUrl == null ? "../images/calendar.gif" : obj.iconUrl ; 
...
//		alert(parentEls.indexOf(calId+",") == -1);alert(event.target.id);alert(obj.id);alert(parentEls.indexOf(calId+"Icon,"));
...
//        $("#" + id.substring(3)).val(this.id.replace(id,""));
		var obj = $("#" + id.substring(3)).parent().parent().parent();
        var date = this.id.replace(id,"");
        if($('#cardD').val()=='card'){//지출 페이지
        	$('#tmDt').val(date);
        	$('#cardDate').html(date);
        	
        } else if($('#pageName').val()=='PrivateWorkVeiw'){//지출 페이지
        	$("#" +id.substring(3)).val(date);
        	//alert(obj.find('[id^=AP_dueDay]').val());
        } else {// deal, mna 페이지에서
			$.ajax({
				type:"POST",        //POST GET
				url:"../work/modifyDueDate.do",     //PAGEURL
				data : ({sNb: obj.find('[id^=offerSnb]').val(), dueDt: date}),
				timeout : 30000,       //제한시간 지정
				cache : false,        //true, false
				success: function whenSuccess(arg){  //SUCCESS FUNCTION
	//					$(obj).hide();
					if($("#SendReceive").val() == 'R'){ 
						document.modifyRec.action = "selectWorkDealR.do";
...
//	if(iconYN == "y"){
//		$(document.body).append("<div id='"+ id +"Icon' style='position:absolute'><img id='"+ id +"IconImg' class='calImg' src='"+ imageUrl +"'><div>");
//	}
