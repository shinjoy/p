	...
	<table class="tb_list_basic" summary="결재문서함 (전체선택, 등록일, 구분, 제목, 등록자, 진행상황)" id = "listTable">
		...
		<tbody>
			<c:forEach items="${companyFormList }" var = "data">
				<tr>
					<td class="e_doc_type"  onclick="goCreatePage('${data.appvCompanyFormId }')">${data.appvDocTypeName }</td>
					<td class="e_doc_type"  onclick="goCreatePage('${data.appvCompanyFormId }')">${data.appvDocNumRuleMidName }</td>
					<%-- <td class="e_doc_type"  onclick="goCreatePage(('${data.appvCompanyFormId }')">${data.openYn eq 'Y'?'공개':'비공개' }</td> --%>
					<%-- <td class="doc_writer"  onclick="goCreatePage(('${data.appvCompanyFormId }')">
					...
					<td class="num_date_type"  onclick="goCreatePage('${data.appvCompanyFormId }')"><fmt:formatDate value="${data.createDate }" pattern="yyyy-MM-dd" /></td>
				</tr>
			</c:forEach>
