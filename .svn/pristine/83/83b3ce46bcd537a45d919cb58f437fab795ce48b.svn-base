...
<div class="doc_viewWarp">
    ...
    <div class="doc_viewWarp2">
        ...
        <h3 class="print_h3title">지출결의서</h3>
        <table class="doc_print_tb_st01" summary="지출결의서 보기(작성자, 신청일, 소속(직위), 연락처, 휴가기간, 휴가기간, 휴가내용, 업무대행자, 대결자, 참조인)">
            <caption>구매신청서 보기</caption>
            <colgroup>
                <col width="130" />
                <col width="*" />
                <col width="120" />
            ...
            <tbody>
                <tr>
					...
					<span class='userProfileTargetArea' onmousedown = 'openUserProfileBox("${detailMap.userId}",$(this),"mouseover",null,0,0,0)'>
						<strong>${detailMap.userNm}(${detailMap.mobileTel})</strong>
					</span>
					</td>
				...
				<tr>
               	...
               	<tr>
               		...
               		<td colspan="3">${detailMap.chargeType eq 'CHARGE'?'청구':detailMap.chargeType eq 'RECEIVE'?'영수':'' }</td>
               	</tr>
                <tr>
                    <th scope="row">제목</th>
                    <td colspan="3">${detailMap.title }</td>
                </tr>
                <tr>
                    <th scope="row">지출내역</th>
                    <td colspan="3">
                        <div class="doc_pointcon">
                            <table class="purchase_view_tb mgt5" summary="구매내역(구매물품, 단가, 수량, 금액)">
                                ...
                                <tbody>
                                	<c:set var = "cashAmount" value="0" />
                                    <c:set var = "cardAmount" value="0" />
                                    <c:forEach items="${detailMap.expenseList}" var = "data">
                                        <tr>
                                            <td>${data.expenseDateStr }</td>
                                            <td>${data.expenseTypeNm }</td>
                                            <td>

                                            	<c:if test="${data.paymentType eq 'CASH' }">
                                            		<c:set var = "cashAmount" value="${cashAmount + data.amount }" />
                                            		현금
                                            	</c:if>
                                            	<c:if test="${data.paymentType eq 'CARD' }">
                                            		<c:set var = "cardAmount" value="${cardAmount + data.amount }" />
                                            		카드
                                            	</c:if>
                                            </td>
                                            <td>${data.summary }</td>
                                            <td class="txt_right"><fmt:formatNumber type="CURRENCY" pattern="#,##0" value="${data.amount}" /></td>
                                            <td>${data.comment }</td>
                                        </tr>
                                    </c:forEach>
                                    <script type="text/javascript">
                                        $("#totalPrice").text("<fmt:formatNumber type='CURRENCY' pattern='#,##' value='${totalCnt }' />");
                                    </script>
                                </tbody>
                            ...
                            <table class="purchase_view_tb mgt5" summary="구매내역(구매물품, 단가, 수량, 금액)">
                                ...
                                <tbody>
                                	<tr>
	                                	<th>소계</th>
										<th>현금</th>
										<td class="txt_money"><fmt:formatNumber type="CURRENCY" pattern="#,##0" value="${cashAmount}" />원</td>
										<th>카드</th>
										<td class="txt_money"><fmt:formatNumber type="CURRENCY" pattern="#,##0" value="${detailMap.amount}" />원</td>
									</tr>
									<tr>
										<th>총계</th>
										<td class="txt_money" colspan="3"><fmt:formatNumber type="CURRENCY" pattern="#,##0" value="${detailMap.amount}" /></td>
										<td>원</td>
									</tr>
                                </tbody>
                ...
                <tr>
                    ...
                    <td colspan="3">
                    	<c:if test = "${not empty detailMap.expenseDay}">
	                    ${detailMap.expenseDay eq '32'?'말':detailMap.expenseDay}일
	                    </c:if>

	                    <c:if test = "${detailMap.advanceWishYn eq 'Y'}">
	                    	(선지불요망)&nbsp;&nbsp;<fmt:formatDate value="${detailMap.advanceWishDate }" pattern="yyyy-MM-dd" />
	                    </c:if>
                    </td>
                </tr>
                <c:if test = "${detailMap.advanceWishYn eq 'Y' and not empty detailMap.advanceWishReason}">
                <tr>
                    <th scope="row">선지급요청사유</th>
                    <td colspan="3">${detailMap.advanceWishReason }</td>
                </c:if>
                <tr>
                    <th scope="row">세금계산서<br>발행일</th>
                    <td colspan="3"><fmt:formatDate value="${detailMap.taxBillIssueDate }" pattern="yyyy-MM-dd" /></td>
                </tr>
                <tr id = "fileViewArea">
                    <th scope="row">
                    	첨부파일
        ...
        <p class="doc_des_st">위와 같이 구매 신청서를 제출합니다.</p>
        ...
        <!-- 신규 수신확인/문서번호 : S -->
		<c:if test="${detailMap.docStatus ne 'WORKING'}">
			<c:if test="${(searchMap.listType eq 'rcList' or searchMap.listType eq 'expenseList') and detailMap.rcReceiptCnt eq 0 and detailMap.totReceiptCnt > 0 and detailMap.receiverUserYn eq 'Y'}">
				<span class="doc_read_check"><a href="javascript:processAppvRc()" class="btn_s_type_red">&nbsp;수신확인&nbsp;</a></span>
			</c:if>
			<c:if test="${(searchMap.listType eq 'rcList' or searchMap.listType eq 'expenseList') and detailMap.rcReceiptCnt >0 }">
		...
		<c:if test="${searchMap.listType eq 'expenseList' and detailMap.expenseYn ne 'Y'}">
			<span class="doc_read_check"
				<c:if test="${ detailMap.totReceiptCnt >0 and detailMap.receiverUserYn eq 'Y' }">style="margin-top: 40px;"</c:if>
			><a href="javascript:openProcessExpense('Y')" class="btn_s_type_red">&nbsp;미지급&nbsp;</a></span>
		</c:if>
		<c:if test="${searchMap.listType eq 'expenseList' and detailMap.expenseYn eq 'Y'}">
			<span class="doc_read_check"
			<c:if test="${ detailMap.totReceiptCnt >0 }">style="margin-top: 40px;"</c:if>
			><a href="#this" class="btn_s_type_red">&nbsp;지급:${detailMap.expenseUserNm }&nbsp;</a></span>
		</c:if>
    </div>
</div>
<!--문서미리보기-->