...
public class OrgCompanyRegServiceImpl implements OrgCompanyRegService{
	...
	public void setOrgCompanyInfo(Map param) throws Exception {
		...
		insertMap.put("secomeUseYn", param.get("secomeUseYn"));			//세콤사용여부
		...
		userMap.put("secomeUseYn", param.get("secomeUseYn")); 								//세콤연동여부
	...
	public void setOrgAuthCompany(Map param)  throws Exception{
		...
		//171103 psj 수정 : 이제 소속 관계사 권한도 변경할수 있다.
		deleteMap.put("allDelete", "Y");
		...
		for(int i = 0 ; i < jsonArr.size() ; i++){
			...
			if(!StringUtils.isEmpty(authChk)){
				...
				//171103 psj 수정 : 이제 소속 관계사 권한도 변경할수 있다.
				orgCompanyRegDAO.insertBsRelationCompanyAccess(map);
			}
	...
	public List<EgovMap> getRelationCompanyAccessAuthListForOrg(Map param) throws Exception{
		return orgCompanyRegDAO.getRelationCompanyAccessAuthListForOrg(param);
	}
}