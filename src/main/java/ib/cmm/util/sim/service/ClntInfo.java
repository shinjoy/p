...
public class ClntInfo {
	...
	*/
	public static String getClntOsInfo(HttpServletRequest request) throws Exception {
		...
		String os_conf = IBsProperties.getProperty(Globals.CLIENT_CONF_PATH, os_info.replaceAll(" ", ""));
		String osInfo = "";
	...
	*/
	public static String getClntWebKind(HttpServletRequest request) throws Exception {
		...
		if (user_agent.toUpperCase().indexOf("CHROME") != -1) {
			...
			webKind = "iPhone";
		} else if (user_agent.toUpperCase().indexOf("ANDROID") != -1) {
			webKind = "Android";
	...
	*/
	public static String getClntWebVer(HttpServletRequest request) throws Exception {
		...
		// 웹브라우저 버전 조회
		String webVer = "";int tmp=0;
		String [] arr = {"CHROME", "IPHONE", "ANDROID", "MSIE", "OPERA", "NETSCAPE", "FIREFOX", "SAFARI"};
		for (int i = 0; i < arr.length; i++) {
			int s_loc = user_agent.toUpperCase().indexOf(arr[i]);
			if (s_loc != -1) {
				int f_loc = s_loc + arr[i].length();tmp=f_loc;
				...
				System.out.println("\n\n agent::: "+user_agent+" webkind:::  "+user_agent.toUpperCase().substring(tmp, tmp+5)+"\n\n");
				return webVer;
			}
		}return null;
	}
