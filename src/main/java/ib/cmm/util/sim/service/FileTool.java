...
import ib.cmm.WebUtil;
import ib.cmm.service.Globals;
import ib.cmm.service.IBsProperties;
...
public class FileTool {
	...
	File f = null;
	try {
	    f = new File(WebUtil.filePathBlackList(targetDirPath));
	...
	try {
	    f = new File(WebUtil.filePathBlackList(baseDirPath));
	    isExist = f.exists();
		...
		f = new File(WebUtil.filePathBlackList(subDirPath));
		...
		if (targetDirPath.equals(dirList[i])) {
		    ...
		    if (new File(WebUtil.filePathBlackList(baseDirPath) + "/" + dirList[i]).isDirectory()) {
			dirFindCnt++;
	...
	String result = "";
	try {
	    f = new File(WebUtil.filePathBlackList(filePath));
	...
	try {
		...
		childFile = new File(WebUtil.filePathBlackList(subDirPath));
	...
	File file = new File(WebUtil.filePathBlackList(dirPath));
	String result = "";
	try {
	    //            if(file.exists()){
	    //            	//log.debug("File Exist..");
	...
	File srcDrctry = new File(WebUtil.filePathBlackList(drctry));
	...
	for (int i = 0; i < fileArray.length; i++) {
		...
		if (Globals.OS_TYPE.equals("UNIX")) {
		    String[] command = { IBsProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".getDrctryByOwner"),
	...
	String result = "";
	File file = new File(WebUtil.filePathBlackList(dirDeletePath));
	if (file.isDirectory()) {
		...
		File f = new File(WebUtil.filePathBlackList(dirDeletePath) + "/" + fileList[i]);
		if (f.isFile()) {
	...
	try {
	    ...
	    File srcFile = new File(WebUtil.filePathBlackList(file1));
	...
	File file = new File(WebUtil.filePathBlackList(drctry1));
	...
	File file = new File(WebUtil.filePathBlackList(parFile1));
	...
	File file1 = new File(WebUtil.filePathBlackList(cmprFile11));
	File file2 = new File(WebUtil.filePathBlackList(cmprFile22));
	...
	try {
		...
		//log.debug("tar:"+tar);
		File tarFile = new File(WebUtil.filePathBlackList(tar));
	...
	for (int i = 0; i < source.length; i++) {
	    ...
	    File chkFile = new File(WebUtil.filePathBlackList(src));
	    if (!chkFile.exists()) {
		//log.debug("+++ 원본 파일이 존재하지 않습니다.");
		return result;
	    }
	...
	for (int j = 0; j < source.length; j++) {
		...
		// 타겟파일이름 명명
		File chkFile = new File(WebUtil.filePathBlackList(source[j]));
		String tarTemp = tar + FILE_SEPARATOR + chkFile.getName();

		try {
		    // 복사될 target 파일 생성
		    tarTemp = createNewFile(tarTemp);
		    File tarFile = new File(WebUtil.filePathBlackList(tarTemp));
	...
	if (srcFile.exists() && srcFile.isDirectory()) {
		...
		if (result) { //f(result != false){
		    ...
		    // 타겟파일이름 명명
		    File chkFile = new File(WebUtil.filePathBlackList(abspath));
		    String tarTemp = tar + FILE_SEPARATOR + chkFile.getName();
	...
	try {
		...
		tarFile1 = new File(WebUtil.filePathBlackList(tarFile1.getAbsolutePath()) + "/" + srcFile.getName());
	    }
	...
	try {
		...
		if (Globals.OS_TYPE.equals("UNIX")) {
		    Process p = null;
		    String[] command = { IBsProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".getMountLc"), src, "/" };
		    p = Runtime.getRuntime().exec(command);
		    //p.waitFor();
	...
	try {
		...
		Process p = null;
		String cmdStr = IBsProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".getDrctryOwner");
	...
	try {
		...
		// 접근권한 조회
		String parentPath = WebUtil.filePathBlackList(srcFile.getParent());
		String fname = WebUtil.filePathBlackList(srcFile.getName());

		Process p = null;
		if (Globals.OS_TYPE.equals("UNIX")) {
		    String[] command = { IBsProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".getDrctryAccess"),
			    parentPath, fname };
		    p = Runtime.getRuntime().exec(command);
		    p.waitFor();
		    //프로세스 에러시 종료
	...
	try {
		...
		if (strArr.length >= 2) {
		    ...
		    type = IBsProperties.getProperty(Globals.FILE_FORMAT_PATH, format);
		}
	...
	File f = null;
	try {
	    f = new File(WebUtil.filePathBlackList(originalDirPath));
	    // 원본이 유효해야 진행한다.
	    if (f.exists() && f.isDirectory()) {
		...
		if (targetDirPath1.equals("")) {
		    ...
		    File targetDir = new File(WebUtil.filePathBlackList(targetDirPath1));
		    targetDir.mkdirs();
		    // 디렉토리에 속한 파일들을 복사한다.
		    String[] originalFileList = f.list();
		    if (originalFileList.length > 0) {
			for (int i = 0; i < originalFileList.length; i++) {
			    File subF = new File(WebUtil.filePathBlackList(originalDirPath) + FILE_SEPARATOR + originalFileList[i]);
			    if (subF.isFile()) {
	...
	File f = new File(WebUtil.filePathBlackList(targetDirPath));
	if (!f.exists()) {
	    return 0;
	}
	if (f.isFile()) {
	...
	try {
	    ...
	    if (f.exists()) {
		// 타겟으로 설정한 경로가 유효한지 확인(중간경로에 파일명 이 포함되어있으면 유효하지 못하므로 진행안함.
		File targetDir = new File(WebUtil.filePathBlackList(targetDirPath));
		if (targetDir.exists()) {
		    // 타겟경로가 이미 있는 경우는 종료
		    result = false;
		} else {
		    // 새로 생성되는 경우만 진행한다. (이동쉘을 실행시킨다.)
		    String cmdStr = IBsProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".moveDrctry");
		    String[] command = { cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR),
	...
	try {
		...
		if (logF.exists()) {
			...
			FileMntrg t = new FileMntrg(WebUtil.filePathBlackList(targetDirPath), logF);
			t.start();
		    }
		} else {
		    result = logF.createNewFile();
		    FileMntrg t = new FileMntrg(targetDirPath, logF);
		    t.start();
		}
	    }
	} catch (Exception e) {
