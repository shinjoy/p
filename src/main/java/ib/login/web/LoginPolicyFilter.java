...
import ib.login.service.LoginPolicyService;
import ib.login.service.LoginPolicyVO;
...
public class LoginPolicyFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		LoginPolicyService egovLoginPolicyService = (LoginPolicyService) act.getBean("LoginPolicyService");	
		IBsMessageSource MessageSource = (IBsMessageSource)act.getBean("IBsMessageSource"); 
		...
		try {
			...
			if (loginPolicyYn) {
				...
				((HttpServletRequest) request).setAttribute("message", MessageSource.getMessage("fail.common.login"));
				((HttpServletResponse) response).sendRedirect(httpRequest.getContextPath() + "/uat/uia/egovLoginUsr.do");
			}
