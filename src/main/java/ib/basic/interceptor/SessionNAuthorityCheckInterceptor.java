...
public class SessionNAuthorityCheckInterceptor extends HandlerInterceptorAdapter{	
	
	protected static final Log logger = LogFactory.getLog(SessionNAuthorityCheckInterceptor.class);
	...
	public boolean preHandle(HttpServletRequest request,
		...
		logger.debug("######## SessionNAuthorityCheckInterceptor.preHandle() ############");
		...
		if(baseLoginUser == null) {			
			throw new HttpSessionRequiredException(iBsMessageSource.getMessage("fail.session.out"));
		}
		...
		logger.debug("######## SessionCheckInterceptor[ Check RequestURI :"+ requestURI +" ] #########");
		
		
		//메뉴 권한체크
		String menufilter = (String) session.getAttribute("menuFilterStr");		//허용메뉴 리스트 문자열
		logger.debug("###############메뉴 필터 :"+ menufilter.toString());
		if(menufilter.indexOf(requestURI) == -1 || "".equals(menufilter)){		//허용메뉴에 존재하지 않는 요청이 온 경우
