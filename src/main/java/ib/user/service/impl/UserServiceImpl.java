...
public class UserServiceImpl extends AbstractServiceImpl implements UserService {
	...
	public List<EgovMap> getUserModuleList(Map<String,Object> map) throws Exception{
		return userDAO.getUserModuleList(map);
	}
	...
	public Integer processUserMainModule(Map<String,Object> map) throws Exception{
		...
		String updateUserModuleStr = map.get("updateUserModuleStr").toString();
		String[] updateUserModuleStrBuf = updateUserModuleStr.split(",");
		if(updateUserModuleStrBuf.length>1){
			userDAO.deleteUserModule(map);

			Map<String,Object> insertMap = new HashMap<String, Object>();
			insertMap.put("applyOrgId", map.get("applyOrgId"));
			insertMap.put("userId", map.get("userId"));
			insertMap.put("roleId", map.get("roleId"));

			for(String valueArr :updateUserModuleStrBuf){
				String[] valueBuf = valueArr.split("[|]");

				if(valueBuf.length>1){
					String coordinateXx = valueBuf[0];
					insertMap.put("coordinateXx", coordinateXx);
					for(int i = 1;i<valueBuf.length;i++){
						insertMap.put("coordinateYy", i-1);

						insertMap.put("moduleId", valueBuf[i]);

						cnt=userDAO.insertUserModule(insertMap);
					}
				}
			}
