...
<head>
<meta charset="utf-8">
<title>회사일괄등록 | 회사 | 네트워크 | PASS (Project based Activity analysis and Sharing system)</title>
...
<script>
	...
	function doUpload() {
		...
		//commonAjaxSubmit("POST",$("#fileForm"),callback);

		//commonAjaxForFileCreateForm(url,"","fileUrl","10","fileSize", callback , "");

		$('#fileForm').attr('action', "<c:url value='/worktime/uploadWorktimeExcel.do'/>").submit();
	}
	...
	//csv양식다운로드
	function downTmpl(){
		var downPop = window.open('<c:url value="/data/excelTmpl/worktimeExcelTemplate.xlsx" />');
	}
...
<input type="hidden" name="fileSize" value="5">

<div id="compnay_dinfo">
		<!--회사상세보기(회사정보)-->
		<div class="modalWrap2">
			<h1><strong>[출근부 엑셀 업로드]</strong> 출근부업로드</h1>
			<div class="mo_container">
				<h3 class="h3_title_basic"><span>출근부 엑셀 업로드</span></h3>
				<div class="cvsuploadWrap">
					<div class="re_CompanyList">
						<ul class="gray_dot_list">
							<li><span>출근부정보를 엑셀양식에 맞춰 작성 후 업로드 하시면 출근부등록 및 수정이 일괄적으로 적용됩니다.</span><button type="button" class="btn_b_skyblue mgl10" onclick="downTmpl()"><em class="icon_down">엑셀양식 다운</em></button> <a href="#"></a></li>
							<li>
							    <span>파일은 <strong class="fontB">반드시 지정된 엑셀파일형식</strong>이여야 합니다.</span>
							    <!-- <button type="button" onClick="javascript:showlayer('cvsGuideBox')" class="btn_b_skyblue mgl10"><em>등록가이드</em></button> -->
						</li>
						</ul>
				...
				<div id="CompanySerach">
					<table class="tb_ProfileInfo" summary="회사검색">
						...
						<tbody>
							<tr>
								<th scope="row" rowspan="2" class="bgGray"><label for="company">출근부 엑셀파일</label></th>
								<td>
