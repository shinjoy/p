...
<script type="text/JavaScript" src="<c:url value='/js/part/memo.js?ver=0.2'/>" ></script>
...
function mainMemoChkOK(th, stts){
	...
	var url = "${pageContext.request.contextPath}/work/checkMemo.do";
	var fn = function(){
...
function checkMainTable(th,cd,snb){
	...
	var url = "${pageContext.request.contextPath}/main/checkMainTable.do";
	var fn = function(){
...
function memoDivAjax_new(th,e,snb,mainSnb){
	...
	ajaxModule(DATA,"${pageContext.request.contextPath}/main/privateMemo.do",fn);
} */
...
/* function sndMemo_btnOk(th,f){
	...
	,url = "${pageContext.request.contextPath}/work/insertMainNewMemo.do"
	,fn = function(){
...
/* function popUpNew(num,flag,nm,snb){
	...
	if(flag=='new_person') {
		sUrl = "${pageContext.request.contextPath}/person/rgstCst.do";
		sUrl+='?f='+flag+'&n='+num;
		sUrl+='&fromMain=y';
		w='690';
		h='400';
...
.frt_cbeb {height: 300px;margin-bottom: 10px;  } /*cb*/
.frt_mna{height:300px; margin-bottom: 10px; } /*mna*/
.frt_ipo { height: 180px; margin-bottom: 10px; } /*프리*/

.frt_data { height:396px; margin-bottom:10px; } /*정보정리*/
.frt_keyPt { height:396px;  } /*정보정리핵심체크*/

.frt_newNet { height:396px; margin-bottom:10px;  } /*네트워크*/
.frt_newCst { height: 396px; } /*인물추가*/

.frt_cmnt { height:396px; margin-bottom:10px; } /*코멘트*/
.frt_majorMemo { height:396px;  } /*메모*/
...
<div style="overflow:auto;">
	...
	<div id="offerDiv" style="position:relative;"><div id="offerPr" style="display: none;"></div></div>

	<div class="line2">
		<!-- cb / eb -->
		<div class="frt_cbeb">
	        <div class="mainBoxTitle"><span class="title">CB / EB</span><span class="btnZone"><a href="javascript:goMenuByMoreBtn('${pageContext.request.contextPath}/work/selectWorkAllDeal.do', '');"><img src="${pageContext.request.contextPath}/images/main/btn_more_st01.gif" alt="더보기"/></a></span></div>
	        <div class="frt_body">
		            ...
		            <ul class="mg0 pd0 hov bg${ofer.progressCd}" style="width:100%;overflow: hidden;">
		                <nobr>
		                    ...
		                    >
		                        ...
		                        <c:if test="${not empty ofer.realNm}"><img src="${pageContext.request.contextPath}/images/file/files.png" align="absmiddle"></c:if>
		                    </div>
		...
		<!-- M & A -->
		<div class="frt_mna">
	        <div class="mainBoxTitle"><span class="title">M &amp; A</span><span class="btnZone"><a href="javascript:goMenuByMoreBtn('${pageContext.request.contextPath}/work/selectWorkAllDeal.do', '00008');"><img src="${pageContext.request.contextPath}/images/main/btn_more_st01.gif" alt="더보기"/></a></span></div>
			<div class="frt_body">
				<c:forEach var="ofer" items="${offerList2}" varStatus="status">
		...
		<!-- 프리IPO -->
		<div class="frt_ipo">
			<div class="mainBoxTitle"><span class="title">프리IPO</span><span class="btnZone"><a href="javascript:goMenuByMoreBtn('${pageContext.request.contextPath}/work/selectWorkAllDeal.do', '00012');"><img src="${pageContext.request.contextPath}/images/main/btn_more_st01.gif" alt="더보기"/></a></span></div>
			<div class="frt_body">
	...
	<div class="line4">
		...
		<%-- <div class="frt_newCst">
	        <div class="mainBoxTitle"><span class="title">인물추가 / 수정</span><span class="btnZone"><a href="javascript:popUpNew('_0','new_person','${cst.cstNm}','${cst.sNb}');"><img src="${pageContext.request.contextPath}/images/main/btn_add_st03.gif" alt="추가"/></a><a href="javascript:goMenuPerson();"><img src="${pageContext.request.contextPath}/images/main/btn_more_st03.gif" alt="더보기"/></a></span></div>
			<div class="frt_body hov">
				<c:forEach var="net" items="${netList}"><c:if test="${net.netYn == '인물수정' or net.netYn == '인물추가' }">
				<ul class="mg0 pd0 hov" style="width:100%;overflow:hidden;">
					<nobr>
						<div class="mg0 hov" onclick="newCstNnetwork(this,'2','${net.sNb}');" title="${net.position1st}" style="overflow:hidden;width:190px;float:left;padding:0 0 0 5px;"><small>${fn:split(net.netYn,'인물')[0]}</small><font color="navy">&nbsp;${net.cstNm1st}&nbsp;</font><small>${net.cpnNm1st}</small></div>
		...
		<!-- idea제안 -->
		<div class="frt_idea">
			<span class="frt_title">IDEA 제안</span>
			<div class="frt_body">
				<c:forEach var="idea" items="${IdeaCheckList}" varStatus="stat">
				<ul class="mg0 pd0 hov bgOdd" style="width:100%;overflow:hidden;">
					<nobr>
						<div class="mg0" style="overflow:hidden;width:204px;float:left;padding:0 0 0 5px;">
							 - ${idea.title}
						</div><div class="nameNdate hov">&nbsp;</div>
						<div class="mg0" style="overflow:hidden;width:204px;float:left;padding:0 0 0 5px;">
							<span style="visibility:hidden;"> - </span><small><font color="#084B8A">
							<c:if test="${idea.tmpNum1 =='c'}">${idea.text} : ${idea.rcmdReason}</c:if>

							<c:if test="${idea.tmpNum1 =='n'}">
								<c:if test="${empty idea.cstNm}">
									<c:if test="${idea.score != 0}">${idea.text} : ${idea.score} ${idea.prize}</c:if>
									<c:if test="${idea.score == 0}">내용 수정</c:if>
								</c:if>
								<c:if test="${not empty idea.cstNm and idea.rgNm != '구자형'}">
									${idea.cstNm}
								</c:if>
							</c:if>
							</font></small>
						</div>
						<div class="nameNdate"><small>${idea.rgNm} | '${fn:replace(fn:substring(idea.tmDt,2,10),'-','.')}</small></div>
					</nobr>
				</ul>
				</c:forEach>
			</div>
	...
	<div class="line3">
		...
		<div class="frt_cmnt">
			...
			<div class="frt_body">
				<c:forEach var="rp" items="${replyList}" varStatus="memoS">
					...
					<ul class="mg0 pd0 hov" style="width:100%;overflow: hidden;">
						<nobr>
							<div class="mg0 hov" style="overflow:hidden;width:204px;float:left;padding:0 0 0 5px;" onclick="popUp('${rp.rgDt}','rcmdComment','','${rp.offerSnb}');">
								<font color="navy">
								<c:choose><c:when test="${asterDeal eq 'Y'}">***</c:when><c:otherwise>${rp.cpnNm}</c:otherwise></c:choose>
								</font>
								<font color="green">${rp.categoryNm}</font> <font color="#5A5A66">${rp.comment}</font></div>
		...
		<div class="frt_majorMemo">
	        <div class="mainBoxTitle"><span class="title">업무전달 (메모) <a href="javascript:openMemoSearchPop();" style="float:right;"><img src="<c:url value='/images/work/btn_memo_search.jpg' />"  style="vertical-align:bottom;" title="메모상세보기"></a></span><!-- <span class="btnZone"><a href="javascript:memoDivAjax_new(this,event,'0','0');"><img src="${pageContext.request.contextPath}/images/main/btn_add_st04.gif" alt="추가"/></a></span> --></div>
			<div class="frt_body">
				<c:forEach var="memo" items="${memoList}" varStatus="memoS">
				<ul class="mg0 pd0 hov<c:if test="${baseUserLoginInfo.userName != memo.cstNm}"> bgRed</c:if>" style="width:100%;overflow:hidden;">
					<nobr>
						<div class="mg0 hov clr${memo.importance}" onclick="memoDivAjax(this,event,'${memo.sNb}'<c:if test="${memo.sttsCd=='00001' or memo.sttsCd=='00002'}">,'${memo.mainSnb}'</c:if>);" style="overflow:hidden;width:190px;float:left;padding:0 0 0 5px;">
						<c:choose>
						...
						</c:choose><c:if test="${memo.priv eq 'Y'}"><b><font style="background:#FAAC58;">[S]</font></b> ******</c:if>
