		...
		<tbody>
			<tr>
				...
				<td class="no_leftline"><span id="progress_list">
					...
					<input type="hidden" id="progressCodeListCnt" value="${fn:length(progressCodeList)}"/>
				</span>
			</tr>
		</tbody>
	</table>
...
var fnObj = {
	    		...
	    		for(var i = 0 ;i < list.length ;i++){
	    			...
	    			if(item.deleteFlag == 'N'){
	    				if(keyCode == "INFO_PATH"){
	    					...
	    					if(keyCode == "INFO_PROGRESS") progressCodeListCnt++;
	    				}
	    			}
	    		}

	    		if(keyCode == "INFO_PATH"){
	    			html += '<input type="hidden" id="pathListCnt" value="'+pathListCnt+'"/>';
	    			$("#path_list").html(html);
	    		}else if(keyCode == "INFO_TYPE"){
	    			$("#type_list").html(html);
	    		}else if(keyCode == "INFO_PROGRESS"){
	    			//유형인 경우
	    			html += '<input type="hidden" id="progressCodeListCnt" value="'+progressCodeListCnt+'"/>';
	...
	doSave : function(){
		...
		if($("#pathListCnt").val() == 0){
			alertM("정보 분류 코드를 입력해주세요.");
			return;
		}

		if($("#progressCodeListCnt").val() == 0){
			alertM("진행상황 코드를 입력해주세요.");
			return;
		}
