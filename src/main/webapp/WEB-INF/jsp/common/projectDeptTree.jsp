...
<script type="text/javascript">
	...
	//선택된 orgId
	var g_selectOrgId = '${baseUserLoginInfo.applyOrgId}';

	//선택된탭
	var g_selectedTab = 'PROJECT';
	...
	function getOrgIdList(){
        ...
        var orgCodeCombo = getCodeInfo('KOR', 'optionValue', 'optionText', null, null, null, '/common/getOrgCodeCombo.do', params);  //ORG코드(콤보용) 호출
        for(var i = 0 ;i < orgCodeCombo.length ;i++){
            var item = orgCodeCombo[i];
            $("<option />",{ value : item.orgId , text : item.orgNm}).appendTo("#selectOrgId");
            var orgId = g_selectOrgId;
	...
	function selectTabZone(type){
		...
		if(g_selectedTab == "PROJECT"){
            if(idProjectAllAreaDisplay) projectDeptTreeSelectedId = "0";
        }else{
            projectDeptTreeSelectedId = "A0";
        }
	...
	function selectOrgId(){
		g_selectOrgId = $("#selectOrgId").val();
		if(g_selectedTab == "PROJECT"){
			if(idProjectAllAreaDisplay) projectDeptTreeSelectedId = "0";
			selectProjectDeptTreeProject(1);
		}else{
			projectDeptTreeSelectedId = "A0";
			selectProjectDeptTreeDept();
		}
	}
	...
	function selectProjectDeptTreeProjectCallback(data){
		...
		}else if($("#firstProjectId").val() != undefined){
			projectDeptTreeProjectReturnFnc($("#firstProjectId").val(),$("#firstProjectName").val(),$("#firstProjectEmployee").val());
            projectTreeSelect($("#firstProjectId").val());
            projectDeptTreeSelectedId = null;
		}
	...
	function selectProjectDeptTreeDept(){
		 ...
		 var param = {
					...
					'oneOrg' : g_selectOrgId	,					//전달받은 한개의 ORG ID
	...
	function selectProjectDeptTreeDeptCallback(result){
		...
		userList.forEach(function(value, index){
			var str ="<span class='t_name'>"+userList[index].text+"</span>";
 			str += "<span class='t_position'>"+userList[index].rankNm+"</span>";
 			str += userList[index].deptManagerYn == "Y" ? "<span class='t_deptM'><em>부서장</em></span>" : "";

 			userList[index].text = str;
		});

		var deptUserList = deptList.concat(userList);

		var allList = [{code:0,deptId:0,deptNm:"전체",deptOrder:"L00_0",deptSeq:0,id:"A0",levelSeq:0,name:"전체",orgId: g_selectOrgId
...
<div class="addAreaZone printOut">
	<div id="treeArea">
		<div class="tnavi_title">
			...
			<span>
			    <select id="selectOrgId" name="selectOrgId" onChange="selectOrgId();"  class="select_b" title="관계사 선택">
			</select>
		</div>
		...
		<form id = "treeFrm" name = "treeFrm" method="post">
			...
			<input type="hidden" id = "orgId" name = "orgId" value="${baseUserLoginInfo.applyOrgId}">
			<input type="hidden" id = "confirm" name = "confirm" value="Y">
