...
function getLastDate(){
	if($('#dateFrom').val()==""||"${detailMap.docStatus}"!="WORKING"){
		return;
	}
	var url = contextRoot + "/approve/getEventLastDay.do";
	var param = {
			period 	: "${detailMap.familyEventPeriod }",
			holidayYn  :"${detailMap.familyEventsHoliday}",
	...
	var callback = function(result){
	    ...
	    weekday[5]="금";
	    weekday[6]="토";
		var stStr = "<span class=\"txt_num\">"+lastDtStr+"</span> ("+weekday[lastDt.getDay()]+")";
		$("#dateToText").html(stStr);
...
function amountConfirm(){
	...
	var amount = $("#amountEvent").val();


	$("#event_amount").text(amount);

	amount = amount.split(",").join("");
	$("#amountEvent").val(amount);
...
<div class="doc_viewWarp">
	...
	<!-- 신규 수신확인/문서번호 : S -->
		...
		<h3 class="print_h3title">경조신청서</h3>
		<table class="doc_print_tb_st01" summary="경조신청서 보기(작성자, 신청일, 소속(직위), 연락처, 경조기간, 경조기간, 경조내용, 업무대행자, 대결자, 참조인)">
			...
			<tbody>
				...
				<tr>
					<th scope="row">경조사</th>
					<td colspan="3">${detailMap.familyEventsNm }</td>
				</tr>
				<tr>
					<th scope="row">경조금액</th>
					<td>

						<div id = "amountArea">
							<span class="txt_num" id = "event_amount">
								<fmt:formatNumber type="CURRENCY" pattern="#,##0" value="${detailMap.amount}" />
							</span>원
							<a id = "amountModifyBtn" class="btn_s_type_g mgl5 printOut" href="javascript:openModify('AMOUNT')" style="display: none;"><em>수정</em></a>
						</div>

						<div id = "amountModifyArea" style="display: none;" class="printOut">
							<input type="text" class="input_mrb number" id = "amountEvent" name = "amountEvent" value="${detailMap.amount }">원
							<a id = "btnFileSave" class="btn_s_type_g mgl5 printOut" href="javascript:openSavePop('AMOUNT')"><em>저장</em></a>
						</div>
					</td>
					<th scope="row">경조휴일</th>
					<td><span class="txt_num">
						<c:choose>
							<c:when test="${detailMap.docStatus eq 'WORKING'}">
								${detailMap.familyEventPeriod }</span>일 <c:if test = "${detailMap.familyEventPeriod ne 0}">(${detailMap.familyEventsHoliday =='Y'?'휴일포함':'휴일미포함' })</c:if>
							</c:when>
							<c:otherwise>
								${detailMap.diffDay }<c:if test = "${empty detailMap.diffDay}">0</c:if></span>일 <c:if test = "${not empty detailMap.diffDay}">(${detailMap.holiday =='Y'?'휴일포함':'휴일미포함' })</c:if>
							</c:otherwise>
						</c:choose>
				...
				<tr>
					<th scope="row">경조휴가</th>
					<td><span class="txt_num">
						<input type="hidden" id = "dateFrom" value="${detailMap.dateFrom}">
						<input type="hidden" id = "dateTo" name = "dateTo" value="${detailMap.dateTo}">
						<c:if test="${not empty detailMap.dateFrom}">
							<fmt:formatDate value='${detailMap.dateFrom}' pattern='yyyy-MM-dd'/></span> (<fmt:formatDate value="${detailMap.dateFrom}" type="both" pattern="E"/>)

							~
							<div id = "dateToText" style="display: inline;">
								<span class="txt_num"><fmt:formatDate value='${detailMap.dateTo}' pattern='yyyy-MM-dd'/></span> (<fmt:formatDate value="${detailMap.dateTo}" type="both" pattern="E"/>)
							</div>

						</c:if>
						<c:if test="${empty detailMap.dateFrom}">없음</c:if>
					</td>

					<script type="text/javascript">
						if("${detailMap.docStatus}"=='WORKING'){
							getLastDate();
						}
					</script>
					<th scope="row">경조일자</th>
					<td><span class="txt_num"><fmt:formatDate value='${detailMap.eventDate}' pattern='yyyy-MM-dd'/></span></span> (<fmt:formatDate value="${detailMap.eventDate}" type="both" pattern="E"/>)</td>
				</tr>
				 <tr>
                    <th scope="row">
                    	내용
		...
		<p class="doc_des_st">위와 같이 경조 신청서를 제출합니다.</p>
