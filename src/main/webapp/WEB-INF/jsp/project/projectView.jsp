...
<!-- -------- each page css :E -------- -->
	...
	<section id="detail_contents" class = "projectViewSection">
       ...
       <h3 class="h3_table_title">
       	<span>${baseUserLoginInfo.projectTitle} Management</span>
           ...
           <tbody>
               <tr>
                   ...
                   <td colspan="3">
                   		...
                   		<span class="f_right">
							...
							<span class="spe_color_st3"><em>|</em></span>
               ...
               <tr>
                   ...
                   <td>
                   	...
                   	<span id="v_originEndDate" class="mgl10 spe_color_st6 txt_letter0"></span>
                   </td>
                   <th scope="row">마감일</th>
                   <td>
                   	<span id="v_closeDate"></span>
                   	<span id="v_originCloseDate" class="mgl10 spe_color_st6 txt_letter0"></span>
                   </td>
       ...
       <div class="btn_baordZone2" id = "viewBtnArea">
       	   <!-- 확정된 프로젝트에서 노출 (마감일전?)-->
	       <a id="btnPendingProject" style="display: none;" href="javascript:;" onclick="fnObj.processStatus('PENDING');" class="btn_witheline">보류</a>
	       <a id="btnStopProject" style="display: none;" href="javascript:;" onclick="fnObj.processStatus('STOP');" class="btn_witheline">중단</a>

	       <!-- 확정된 프로젝트중 보류나 중단 -->
	       <a id="btnPendingProjectCancel" style="display: none;" href="javascript:;" onclick="fnObj.processStatus('PENDING_CANCEL');" class="btn_witheline">보류취소</a>
	       <a id="btnStopProjectCancel" style="display: none;" href="javascript:;" onclick="fnObj.processStatus('STOP_CANCEL');" class="btn_witheline">중단취소</a>
           ...
           <a href="javascript:;" onclick="fnObj.editProject();" style="display: none;" id="editBtn" class="btn_blueblack btn_auth">수정</a>
           <a href="javascript:;" onclick="fnObj.copyProject();"  class="btn_blueblack btn_auth">복사</a>
           <a href="javascript:;" onclick="fnObj.deleteProject();" style="display: none;" id="deleteBtn" class="btn_witheline btn_del btn_auth" style="color:red!important;">삭제</a>
           <%--<a href="${pageContext.request.contextPath}/project/templateMgmt.do?me=30&mu=27&mr=26&v=2" class="btn_witheline">목록</a> --%>
           <a href="javascript:;" onclick="fnObj.listProject();" class="btn_witheline">목록</a>
       </div>
...
var fnObj = {
	...
	preloadCode: function(){
		...
		codeYesNoForNm = {N:'N',Y:'<strong class="f_redPoint\">Y</strong>'};					//화면에 NM 뿌려주기위해
		...
		codeEmployeeForNm = {A:'<b>전 직원</b>',Y:'<strong class="f_redPoint\">Y</strong>'};
    	...
    	var configObj = {
            ...
            {key:"namelvl",			formatter:function(obj){
            							...
            							for(var i = 0 ; i <empList.length;i++){
            								...
            								if((empObj.leaderYn == "Y" && g_myAuthIds.indexOf('|'+empObj.userId+'|') != -1)||"${baseUserLoginInfo.activityTitle}" == "SUPER"){
    	...
    	var callback = function(result){
    		...
    		if(obj.result == "SUCCESS"){
	    		...
	    		if(project.confirm == 'Y'){
	    			...
	    			if( g_myAuthIds.indexOf('|'+project.createdBy+'|') != -1||'${baseUserLoginInfo.orgBasicAuth}' == "SUPER"){
    	    	...
    	    	$('#createArea').html('<em class="f_blackPoint">등록: </em>'+project.createNm+' ('+(project.createDate).replace(/-/gi,'/')+')');
    	    	$('#updateArea').html('<em class="f_blackPoint">최종수정: </em>'+project.updateNm+' ('+(project.updateDate).replace(/-/gi,'/')+')');
  ...
  //프로젝트 복사 버튼 클릭
    ...
    deleteProject : function(){
    	...
    	if(confirm('[주의!!]\n\n 정말 삭제하시겠습니까?\n')){
    		...
    		var url = "<c:url value='/project/deleteProject.do'/>";
	        var param = {
	        	projectId   : '${param.pId}',
	        	orgId 		: '${baseUserLoginInfo.applyOrgId}',
    ...
    processStatus : function(type){
    	...
    	var url = "<c:url value='/project/processStatus.do'/>";
   		...
   		var param = {
            	...
            	status		: type,
            	orgId		: g_orgId,
            	projectName	: g_projectName,	//프로젝트 이름
            	projectCode : g_projectCode		//프로젝트 코드
