...
var fnObj = {
    ...
    pageStart: function(){
		...
		var configObj = {
				...
				{key : ""			, formatter : function(obj) { 		//이름
										var str = '<strong>'+obj.item.userName+'</strong>';
										 //--- 회사가 다를때 회사명 표시
    ...
    doSearch: function(){
    	...
    	parent.personnelProfileMemoModal.resize();
    ...
    getRoomEntry : function(){
    	...
    	var callback = function(result){
    		...
    		for(var i=0; i<list.length;i++){
    			if(list[i].entryUserId == '${baseUserLoginInfo.userId}') isEntry = true;
     ...
     doSaveEntry : function(memoRoomId){
		 ...
		 if(g_memoRoomId == 0){		//첫글 등록시
			 if(getArrayIndexWithValue(g_newUserList, loginUserId)<0){
				 if('${workUserCnt}' > 1) {  //2017.03.16 전체메모 권한여부의 권한이 있는경우 타인 메모 등록 가능함
		 ...
		 /* for(var i=0 ; i<g_entryUserList.length; i++){
			 if(getRowIndexWithValue(g_newUserList, 'userId', g_entryUserList[i].userId)<0){	//신규 리스트에 이미 참가자로 등록한 사람이 없으면, 삭제
				 deleteUserList.push(g_entryUserList[i].memoRoomEntryId);
			 }
		 } */
    	 ...
    	 var callback = function(result){
    		 if(g_memoRoomId >0){
    			...
    			/* if(parent.fnObj == undefined){
    				 parent.memoObj.getMemoList();
     			}else{
     				parent.fnObj.doSearch(g_workUserId);
     			} */
    	...
    	if(!confirm("해당 글을 삭제하면 참가자 및 모든 목록에서 삭제됩니다. 삭제하시겠습니까?")){
      	...
      	var callback = function(result){
      		...
      		if(cnt == 0){
      			...
      			parent.personnelProfileMemoModal.close();
      			if(parent.fnObj == undefined){
      				 parent.memoObj.getMemoList();
       			}else{
       				parent.fnObj.doSearch(g_workUserId);
